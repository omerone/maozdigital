          ...
          {results.map((result, index) => (
            ...
            >
              <div className="relative">
                <img
                  src={result.image}
                  alt={`תמונה של ${result.title} - ${result.description}`}
                  className="w-full h-48 object-cover cursor-pointer hover:opacity-90 transition-opacity focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                  onClick={() => setSelectedImage({ src: result.image, title: result.title })}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      setSelectedImage({ src: result.image, title: result.title });
                    }
                  }}
                  tabIndex={0}
                  role="button"
                  aria-label={`צפה בתמונה של ${result.title}`}
                />
              </div>
            </div>
          ))}
